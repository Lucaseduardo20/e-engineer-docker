# Task 10 — Usar score técnico para recomendações no cockpit
...
Usar ProjectTechnicalProfile para recomendar KnowledgeItems, projetos, documentos modelo ou checklists no cockpit.

---

# Contexto específico

Agora as recomendações devem considerar força do contexto técnico, não apenas interseção simples.

---

# Escopo funcional

Criar/evoluir `RecommendProjectContextItemsUseCase` ou `RecommendKnowledgeForProjectUseCase`. Retornar recomendações com type, item, score, matchedTags, reason e alreadyApplied.

---

# Backend — O que deve ser feito

Recomendar KnowledgeItems published, projetos semelhantes, document_model e review_checklist usando profile. Excluir archived, penalizar/sinalizar deprecated, reason obrigatório.

---

# Frontend — O que deve ser feito

No cockpit, seção de recomendações contextuais com motivo, ação Aplicar ao projeto e Abrir detalhe. Texto: “Sugestões baseadas no contexto técnico deste projeto.”

---

# Regras de negócio

Mesmo tenant, sem archived, knowledge published, reason sempre, alreadyApplied, sem IA/embeddings.
...
- Endpoint retorna recomendações baseadas no profile
- Tem score/matchedTags/reason
- Frontend exibe no cockpit
- Archived não aparece
- Tenant protegido
- alreadyApplied funciona

---

# QA — Cenários mínimos

- Knowledge com tags fortes aparece primeiro.
- Motivo cita tags.
- Already applied sinalizado/removido.
- Sem profile mostra empty state.
