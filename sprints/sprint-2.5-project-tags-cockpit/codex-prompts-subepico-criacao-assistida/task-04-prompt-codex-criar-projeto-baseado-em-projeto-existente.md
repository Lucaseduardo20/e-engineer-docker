# Task 4 — Criar projeto baseado em projeto existente
...
Permitir que o usuário crie novo projeto usando um projeto existente como base, reaproveitando contexto técnico sem copiar tudo cegamente.

---

# Contexto específico

A e-engineer deve evitar que empresas comecem do zero quando já fizeram algo parecido. O usuário deve escolher um projeto recomendado e usá-lo como base.

---

# Escopo funcional

Criar use case `CreateProjectFromBaseProjectUseCase` e endpoint `POST /projects/from-base`. Body com baseProjectId, name, client, description, tagIds, inheritTags, inheritDeliverables.

---

# Backend — O que deve ser feito

Validar baseProjectId no mesmo tenant, criar novo Project, registrar origem em `project_base_relations` ou relação equivalente, herdar tags se solicitado e registrar activity log se existir.

---

# Frontend — O que deve ser feito

No card de projeto semelhante, ação “Usar como base”. Abrir modal com novo nome, projeto base, opção de manter tags, indicação de revisão posterior e redirecionar ao cockpit.

---

# Regras de negócio

Não copiar documentos/revisões/activity do projeto base. Não copiar responsáveis/prazos cegamente. Registrar origem. Usuário precisa permissão de criar projeto.
...
- Usuário cria projeto baseado em outro
- Base validada por tenant
- Origem registrada
- Tags herdadas opcionalmente
- Não copia histórico indevido
- UI tem ação Usar como base

---

# QA — Cenários mínimos

- Criar projeto a partir de base e validar relação.
- Herdar tags quando opção ativa.
- Base de outro tenant retorna erro seguro.
- Novo projeto não recebe documentos/revisões antigas.
- UI não parece clonagem cega.
