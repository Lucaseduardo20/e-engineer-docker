# Task 2 — Criar ProjectTechnicalProfile
...
Criar o conceito de Perfil Técnico do Projeto, consolidando o contexto técnico de um projeto a partir das tags aplicadas nele.

---

# Contexto específico

O projeto precisa ter uma leitura semântica acumulada. Inicialmente, o profile pode considerar apenas tags diretamente vinculadas ao projeto. Depois será enriquecido por Deliverables, Documents, Reviews e Knowledge.

---

# Escopo funcional

Criar use case `GetProjectTechnicalProfileUseCase`. Output deve retornar projectId, organizationId e tags com score e sources. Inicialmente: tag direta no projeto = +3.

---

# Backend — O que deve ser feito

Criar use case, DTO/response, service de score, endpoint opcional `GET /projects/:projectId/technical-profile`, testes e integração com ProjectTagRepository.

---

# Frontend — O que deve ser feito

Preparar service frontend. Opcionalmente exibir no detalhe: “Contexto técnico do projeto” com principais tags. Não expor score se confundir UX.

---

# Regras de negócio

Profile só considera dados da mesma organizationId. Projeto sem tags retorna vazio. Archived não compõe profile. Score simples, determinístico e explicável. Não vender como IA.
...
- Use case existe
- Profile retorna tags com score
- Endpoint ou decisão documentada
- Respeita tenant
- Omitir archived
- Projeto sem tags retorna vazio
- Score documentado
...
- Projeto com tags retorna score.
- Projeto sem tags retorna vazio.
- Archived não aparece.
- Usuário de outro tenant não acessa.
- Chamadas repetidas retornam mesmo score.
